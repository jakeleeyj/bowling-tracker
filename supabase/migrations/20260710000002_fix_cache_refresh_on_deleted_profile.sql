-- When a user is deleted, the cascade removes their games, which fires the
-- cache-refresh trigger — but the profile row is already gone, so the cache
-- upsert violates its FK. Early-return guard clears the stale cache instead.
CREATE OR REPLACE FUNCTION public.refresh_player_ranking(p_user_id uuid)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
declare
  ...
  i int;
begin
  -- Deleted user: clear cache rows instead of recomputing (FK would fail)
  if not exists (select 1 from profiles where id = p_user_id) then
    delete from player_rankings_cache where user_id = p_user_id;
    return 0;
  end if;
